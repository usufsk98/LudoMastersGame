...
namespace ChartboostSDK {
	...
	public class Chartboost: MonoBehaviour {
		...
		/// <summary>
		/// This method can be used to check if Chartboost has been initialized
		/// </summary>
		...
		/// <summary>
		/// Set an identifier to send in the POST body for all Chartboost API server requests that come from a mediation partner.
		/// </summary>
		/// <param name="mediator">The identifier to send with all Chartboost API server requests.</param>
		/// <param name="version">The mediator version to send with all Chartboost API server requests.</param>
		...
		private void didInitializeEvent(string data) {
			if (didInitialize != null) {
				didInitialize(Convert.ToBoolean(data));
			}
		}
